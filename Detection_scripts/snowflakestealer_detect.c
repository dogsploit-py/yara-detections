/*
 * detect.c — Snowflake stealer detector
 * =====================================
 *
 * Detects the Snowflake infostealer family (two-stage: MSVC x86 TEA loader
 * + Rust x86-64 payload) by static file signatures, known-sample SHA-256
 * hashes and — on Windows — live system artifacts.
 *
 * IOCs were extracted by reverse engineering sample
 *   1ae99a454f6c11e30c346ca825e2d20bc5450ddb808f25dd20a4d952604d34f0  (loader)
 *   9af802039a2d10a04472824411e07e2a5e41754ca9aaf087f461b1c36ce16195  (stage-2 payload)
...
static const char *KNOWN_HASHES[] = {
    /* stage-1 TEA loader (fake GUI app, PE32 x86) */
    "1ae99a454f6c11e30c346ca825e2d20bc5450ddb808f25dd20a4d952604d34f0",
    /* stage-2 Rust x86-64 payload, dumped/decrypted */
...
static const sig_t SIGS[] = {
    /* --- stage-1 loader --- */
    SIGSTR("mutex name 'runexobozez'", "runexobozez", 3),
    /* TEA key at VA 0x4144c0: E9 40 80 43 3F 55 3A 8A 03 6C 4A 65 26 6A 1B 90 */
    { "TEA key bytes (0x438040e9 0x8a3a553f 0x654a6c03 0x901b6a26)",
      (const uint8_t *)"\xe9\x40\x80\x43\x3f\x55\x3a\x8a\x03\x6c\x4a\x65\x26\x6a\x1b\x90",
      16, 3 },

    /* --- stage-2 Rust payload: C2 + config --- */
    SIGSTR("C2 domain rate0000my7777poo.com", "rate0000my7777poo.com", 3),
    SIGSTR("C2 endpoint /loaderRules",  "/loaderRules",  2),
    SIGSTR("C2 endpoint /grabberRules", "/grabberRules", 2),
    SIGSTR("exfil query string tags/pass/cc/hwid",
           "?tags=&pass=&cookie=&cc=&hwid=&bit=&ip=", 3),
    SIGSTR("C2 endpoint /panic", "/panic", 1),

    /* --- stage-2 module strings --- */
    SIGSTR("wallet registry value 'strDataDir'", "strDataDir", 1),
    SIGSTR("old cargo registry build path (github.com-1ecc6299db9ec823)",
           "github.com-1ecc6299db9ec823", 1),
    SIGSTR("Telegram tdata marker", "\\tdata\\", 1),
    SIGSTR("SDA maFile marker", ".maFile", 1),
};
...
static void scan_file(const char *path) {
    ...
    if (hash_hit) {
        printf("       SHA-256 matches known Snowflake sample\n");
        score = 100;
    }
    ...
    if (score >= SCORE_DETECT) {
        printf("[DETECT] %s: Snowflake stealer (score %d)\n", path, score);
        if (g_worst < 2) g_worst = 2;
    } else if (score >= SCORE_SUSPICIOUS) {
        printf("[SUSP ] %s: possible Snowflake variant (score %d)\n", path, score);
...
static int live_checks(void) {
    ...
    /* Snowflake singleton mutex — exists while the loader is running */
    h = OpenMutexA(SYNCHRONIZE, FALSE, "runexobozez");
    if (h) {
        printf("[LIVE ] mutex 'runexobozez' exists — Snowflake loader is ACTIVE (+4)\n");
        score += 4;
        CloseHandle(h);
    } else {
        printf("[LIVE ] mutex 'runexobozez' not present\n");
    }

    /* Snowflake has no documented persistence; nothing further to check. */
    printf("[LIVE ] note: family has no known persistence artifacts; "
           "check for plaintext HTTP POSTs to /files/ on rate0000my7777poo.com\n");

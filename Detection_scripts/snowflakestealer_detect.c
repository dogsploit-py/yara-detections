/*
 * detect.c — Snowflake stealer detector (v5, ROP-based)
 * =================================================
 *
 ...
 * two things:
 *
 *   1. known-sample match: SHA-256 of the analysed Snowflake stealer samples
...
static const char *KNOWN_HASHES[] = {
    /* stage-1 MSVC x86 loader (TEA payload decryptor) */
    "1ae99a454f6c11e30c346ca825e2d20bc5450ddb808f25dd20a4d952604d34f0",
    /* stage-2 Rust x86-64 payload (decrypted/dumped) */
...
static void scan_file(const char *path) {
    ...
    if (hash_hit) {
        printf("[DETECT] %s: Snowflake stealer (known sample, SHA-256)\n", path);
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr,
            "Snowflake stealer detector (v5: known-sample hash + ROP structural scan)\n"
            "usage:\n"
